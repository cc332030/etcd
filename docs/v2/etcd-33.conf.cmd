...
::本机IP地址
set LOCAL_IP=192.168.25.33

::ETCD存储目录
set DATA_DIR=data

:: 用于 etcdctl 命令连接, 其中 127.0.0.1 用于本地连接
set LISTEN_CLIENT_URLS="http://192.168.25.33:2379,http://127.0.0.1:2379"

:: 用于监听其他 etcd member 连接
set LISTEN_PEER_URLS="http://192.168.25.33:2380,http://127.0.0.1:2380"

:: 本地用于监听并连接其他 member 的地址
set INITIAL_ADVERTISE_PEER_URLS="http://192.168.25.33:2380"

:: 用于标记集群唯一性的 token
set INITIAL_CLUSTER_TOKEN="etcd-cluster"

:: 启动集群使, 使用静态连接方法, 定义每个 member 主机名 endpoint
set INITIAL_CLUSTER="node-128=http://192.168.25.128:2380,node-33=http://192.168.25.33:2380"
